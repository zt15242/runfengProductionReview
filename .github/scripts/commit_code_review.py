...
class CommitCodeReviewer:
    ...
    def review_code_with_ai(self, filename, content_diff, status):
        ...
        # 限制代码长度 - 增加限制以容纳完整文件内容
        max_code_length = 8000
        truncated_content = content_diff[:max_code_length]
        if len(content_diff) > max_code_length:
            truncated_content += f"\n... [已截断，原始内容{len(content_diff)}字符]"
